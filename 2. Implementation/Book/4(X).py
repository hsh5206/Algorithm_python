...
캐릭터의 움직임을 설정하기 위해 정해 놓은 매뉴얼은 이러하다.
  ...
  2. 캐릭터의 바로 왼쪽 방향에 아직 가보지 않은 칸이 존재한다면, 왼쪽 방향으로 회전한 다음 왼쪽으로 한 칸을 전진항다.
...
입력:
4 4     -> N x M  
1 1 0   -> (1, 1)에서 북쪽(0)을 바라보고 서 있는 캐릭터
1 1 1 1 -> 여기서부터 지형
1 0 0 1
1 1 0 1
1 1 1 1
...
while True:
    ...
    if d[nx][ny] == 0 and array[nx][ny] == 0:
        ...
        turn_time = 0
        continue
    # 앞에 가보지 않은 칸이 아니거나 바다이면 회전횟수 1 증가
    else:
        turn_time = 1
    # 먼약 네 방향 모두 갈 수 없으면 뒤로갈 자리 저장
    if turn_time == 4:
        nx = x - dx[direction]
        ny = y - dy[direction]
        # 만약 뒤로 갈 자리가 이동 육지이면
